...
**请求示例**:
```
DELETE /api/v1/auth/user/devices/device-002
```
...
**接口描述**:
- `UpdateDeviceActive`: 批量更新设备活跃时间（由 Gateway/Connect 调用，仅更新 Redis 活跃时间）
- `UpdateDeviceStatus`: 更新设备在线状态（由 Connect 在连接建立/断开时调用）

**说明**: 以上均为内部 gRPC 接口，不对外暴露
...
**请求示例**:
```
GET /api/v1/auth/user/online-status/user-uuid-002
```
