...
**请求示例**:
```
GET /api/v1/auth/friend/apply-list?status=0&page=1&pageSize=20
```
...
**响应示例**: 同 5.2（applicant 变为 target）
...
**请求示例**:
```
GET /api/v1/auth/friend/list?page=1&pageSize=100
```
...
- B 的好友列表中仍保留 A，B 可以给 A 发送消息（但会收到请先加好友的提示）
...
|------|------|------|------|
| userUuid | string | ✅ | 好友UUID |
| groupTag | string | ✅ | 标签名(最多32字符,空字符串表示清除,用空格表示分隔) |
