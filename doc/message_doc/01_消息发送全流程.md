...
│          │                │              │                  └──────────────┘
│          │                │ ①解码 Event  │
│          │                │ ②查Redis路由 │  gRPC PushToUser
│          │                │ ③拆分群成员  │ ─────────────────>┌──────────────┐
│          │  WS Binary帧   │ ④Self-Sync   │                  │   Connect    │
│          │ <──────────────┤              │                  │ (gRPC+WS)    │
│          │  Proto解码     └──────────────┘                  │ 纯管道投递   │
│          │  ──heartbeat─>                                   └──────────────┘
└──────────┘
...
| msg-service → Kafka | **Protobuf Binary**（`MsgPushEvent`） | 写入后立即返回 200 OK，**绝不等待推送** |
| Kafka → Push-Job | Kafka Consumer，Protobuf 反序列化 | Push-Job 解码 `MsgPushEvent`，查 Redis 路由表 |
| Push-Job → Connect | **gRPC** `PushToUser` / `PushToDevice` | Push-Job 按路由表精确调用目标 Connect 节点 |
| Connect → 客户端 | WebSocket Binary, `MessageEnvelope` Proto | Connect 纯管道，拿到就往 WebSocket 塞 |
...
```
Client POST /api/v1/auth/messages/send
  │
  ├─ Gateway:
...
── 异步路径（Kafka Consumer，Push-Job Dispatcher）───────────
Push-Job 消费 Kafka "msg.push"
...
msg-service 将落库后的消息序列化为 **Protobuf Binary** 写入 Kafka，Push-Job 消费并路由到 Connect 后推送到客户端。

**Kafka Topic**：`msg.push`（分区键：`conv_id`，保证同一会话的消息在同一 Partition 内串行有序消费）

> **为什么用 conv_id 而不是 receiver_uuid？**  
> 如果用 receiver_uuid 作为分区键，同一会话的连续消息可能被路由到不同 Partition，导致消费速度差异引起客户端乱序（视觉闪烁）。  
> 用 conv_id 保证同一会话的所有消息始终在同一 Partition 内串行排队，Connect 消费时绝对有序。
...
message MsgPushEvent {
  ...
  string device_id     = 2;  // 发送方当前设备 ID（多端同步时排除该设备）
  string type          = 3;  // "MSG_PUSH" | "MSG_RECALL" | "MSG_MARK_READ"
  ConvType conv_type   = 4;  // 1=单聊 2=群聊（Consumer 据此判断扩散策略）
  bytes  data          = 5;  // 按 type 序列化的业务 proto bytes
  string trace_id      = 6;
...
| **落库** | 写 1 条消息 + 更新双方 2 条会话 | 写 1 条消息 + 仅更新群级 MaxSeq（**不更新成员会话**） |
| **Kafka receiver_uuid** | 对端用户 UUID | 群 UUID |
| **Kafka Partition Key** | conv_id | conv_id |
| **Connect 扩散** | `SendToUser(receiver_uuid)` + Self-Sync | 查询群成员 + Self-Sync |
| **写放大** | 无（1 条 Kafka） | 无（1 条 Kafka） |
| **读放大** | 无 | 有（Connect 查群成员） |

**单聊写扩散流程**：
```
msg-service 落库后：
  ├─ 写 1 条 MsgPushEvent：receiver_uuid = 对端 UUID, from_uuid = 发送方 UUID, conv_type = 1
  └─ Connect 消费后：
       ├─ SendToUser(对端 UUID)，推送到对端所有在线设备
       └─ Self-Sync：SendToUser(from_uuid)，排除 device_id，推送到发送方其他在线设备
```

**群聊读扩散流程**：
```
msg-service 落库后：
  ├─ 写 1 条 MsgPushEvent：receiver_uuid = 群 UUID, from_uuid = 发送方 UUID, conv_type = 2
  └─ Connect 消费后：
       ├─ 查询 Redis 群成员列表（key: group:members:{group_uuid}）
       ├─ 遍历成员，过滤 from_uuid（发送者本人）
       ├─ 对每个在线成员 SendToUser(member_uuid)
       └─ Self-Sync：SendToUser(from_uuid)，排除 device_id，推送到发送方其他在线设备
```

> **群聊落库注意事项（读扩散核心约束）**：  
> 群聊发消息时，msg-service **绝对不能** Upsert 所有群成员的 Conversation 记录！  
> 2000 人群发 1 条消息 = 2000 次 DB 写入，严重写放大，这正是读扩散要规避的。  
> 正确做法：只更新群级会话的 `MaxSeq`，成员的未读数在拉取会话列表时通过 `max(0, group.max_seq - member.read_seq)` 动态计算。

> **为什么群聊用读扩散？**  
> 写扩散会导致 N 个群成员产生 N 条 Kafka 消息，千人群单条消息 = 1000 条 Kafka 写入，写放大严重。
> 读扩散只写 1 条 Kafka，Connect 在消费时查询群成员并逾历推送，这个“读”代价可通过 Redis 缓存降低。

### 4.2 Push-Job Dispatcher（Kafka 消费者调度器）

> Push-Job 是独立的 Kafka 消费者服务（也可复用 msg-service 的后台协程组）。
> 它是整条下行链路中**唯一需要知道路由信息和群成员的聪明节点**。
> Connect 只是一个纯管道，拿到 gRPC 请求就往 WebSocket 发送。

```
Push-Job 消费 Kafka "msg.push" 流程
  │
  ├─ proto.Unmarshal(msg.Value) → MsgPushEvent
  ├─ envelope = MessageEnvelope{type, data, seq=0, server_ts, trace_id}
  │
  ├─ ① 确定目标用户列表（按 conv_type 分支）：
  │    conv_type=1 (单聊)：
  │      targets = [receiver_uuid]  // 只有一个目标
  │
  │    conv_type=2 (群聊)：
  │      members = Redis SMEMBERS group:members:{receiver_uuid}  // 群 UUID
  │      targets = members.filter(uuid != from_uuid)  // 过滤发送方
  │
  ├─ ② 查 Redis 路由表，精确找到目标所在 Connect 节点：
  │    for each target_uuid in targets:
  │      routing = Redis HGETALL user:routing:{target_uuid}
  │      // routing = { "device_A": "10.0.0.5:9091", "device_B": "10.0.0.8:9091" }
  │      按 grpc_addr 分组，合并调用同一节点的请求
  │      gRPC → ConnectService.PushToUser(target_uuid, envelope)
  │
  └─ ③ 发送方多端同步（Self-Sync）：
       from_uuid 非空 →
         routing = Redis HGETALL user:routing:{from_uuid}
         过滤掉 device_id（发送方当前设备）
         对剩余设备所在的 Connect 节点调用 PushToDevice(from_uuid, other_device, envelope)
```

> **多端同步（Self-Sync）说明**：  
> 发送方在手机上发了一条消息，同时登录在电脑端的 App 也应立即看到这条消息。  
> Push-Job 消费到 MsgPushEvent 后，除了向目标（对端/群成员）投递，还会查询 `from_uuid` 的路由表，向除 `device_id` 以外的其他在线设备投递一份。  
> 接收端可通过 `MsgItem.from_uuid == 自己` 判断这是自己发的消息（多端同步），而不是对方发的。

#### Redis 路由表设计

Connect 在建连和心跳时维护此 Hash，断连时删除对应 Field：
...
> **为什么不用 ZSet？**  
> 路由表的核心需求是「按 device_id 查 Connect 节点 IP」，Hash 的 O(1) 读写最合适。  
> 心跳超时清理可通过 Connect 断连时主动 `HDEL` 或配合 TTL Key 实现。
...
message MessageEnvelope {
  string type         = 1;  // 消息类型："MSG_PUSH" | "MSG_RECALL" | "MSG_MARK_READ" | "KICKOUT"
  bytes  data         = 2;  // 业务负载（按 type 序列化的 proto bytes）
  int64  seq          = 3;  // 消息序列号，客户端用来去重/排序
  int64  server_ts    = 4;  // 服务端时间戳（unix 毫秒）
  string trace_id     = 5;  // 链路追踪 ID
  bool   ack_required = 6;  // 是否需要客户端回执
}
```

`type="MSG_PUSH"` 时，`data` 反序列化为 `MsgItem`（定义见 `proto/msg/msg_common.proto`）。
...
收到 WebSocket Binary 帧
  ...
  ├─ 2. 按 type 字段路由：
  │      "MSG_PUSH"      → Unmarshal data → MsgItem，渲染消息气泡
  │      "MSG_RECALL"    → Unmarshal data → RecallNotice，替换气泡
  │      "MSG_MARK_READ" → Unmarshal data → MarkReadNotice，清除红点
  │      "KICKOUT"       → 跳转登录页
  └─ 3. 若 ack_required=true，回送 ACK
```
...
```
POST /api/v1/auth/messages/batch-get
Authorization: Bearer <access_token>
Content-Type: application/json
```
...
> **注意**：`operator_uuid`（撤回操作者 UUID）不在请求体中传递，由 Gateway 从 JWT 中提取后通过 gRPC metadata 透传给 msg-service。Proto 中对应字段为 `RecallMessageRequest.operator_uuid`。
...
- 已读同步：调用 MarkRead 后，服务端向该用户在线设备推送 `MSG_MARK_READ` 通知。
